...
model_name = 'porto_seguro_dae00x4'
data_path = '/home/tom/data/kaggle/porto_seguro_dae'
model_path = os.path.join(data_path, model_name)
if not os.path.exists(model_path):
    os.makedirs(model_path)
train_filename = 'train_rankgauss_'+model_name+'.tfrecord'
valid_filename = 'valid_rankgauss_'+model_name+'.tfrecord'
test_filename = 'test_rankgauss_'+model_name+'.tfrecord'
...
y_train, y_valid = y_0[train_index], y_0[valid_index]
...
with tf.python_io.TFRecordWriter(tfrecord_filename) as tfrecord_writer:
    for i in range(len(y_train)):
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'rankgauss_feature': tf.train.Feature(float_list = tf.train.FloatList(value=X_train[i].tolist()))
        }))
        tfrecord_writer.write(example.SerializeToString())

# generate trainset with label & features
tfrecord_filename = os.path.join(data_path, valid_filename)
# tfrecord_writer = tf.python_io.TFRecordWriter(tfrecord_filename)
with tf.python_io.TFRecordWriter(tfrecord_filename) as tfrecord_writer:
    for i in range(len(y_valid)):
        example = tf.train.Example(features=tf.train.Features(feature={
            'label': tf.train.Feature(int64_list = tf.train.Int64List(value=[y_valid[i]])),
            'rankgauss_feature': tf.train.Feature(float_list = tf.train.FloatList(value=X_valid[i].tolist()))
        }))
        tfrecord_writer.write(example.SerializeToString())
