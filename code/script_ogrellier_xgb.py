...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(target, target)):
    ...
    best_round = int(np.argsort(xgb_evals[:, fold_])[::-1][0]) # TODO: 这块加了int()的小修改，见作者kernel下面的讨论
...
sub_df[["target"]].to_csv(base_path + "test_ogrellier_xgb.csv", index=True, float_format="%.9f")
# Full OOF score : 0.284952
# Best mean score : 0.284657 + 0.004102 @ 192
