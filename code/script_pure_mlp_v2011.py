...
tf_param = {
    "layers": [1500,800,200], "drop": 0.5, "noise_stddev": 0.2 # in keras its drop, in tf its keep
    # "layers": [4000,2000,500,100], "drop": 0.5, "noise_stddev": 0.2 # 特别大的nn增加的capacity对数据空间的流形作用不大，而且增加了训练难度
}

in_dim = X_0.shape[1]
BATCH_SIZE = 64
LR_G = 0.0001#0.0001           # learning rate for generator
LR_D = 0.0001#0.0001           # learning rate for discriminator
...
train_D = tf.train.RMSPropOptimizer(LR_D).minimize(
...
for i1, (train_index, test_index) in enumerate(skf.split(X_0,y_0)):
    ...
    for step in range(n_steps):
        ...
        if step % (3 + 1) == 0: # every n times train one time Gloss; if step % (1 + 1) > 0 # every n times train one time Dloss
...
# CV gini score: 0.253428, score var:1.5071793510471823e-05
# with GAN, RMSPropOptimizer, LR_G=LR_D=0.0001
