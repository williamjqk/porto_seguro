...
test[['id','target']].to_csv('xgb_submission_zusmani.csv', index=False, float_format='%.5f')
...
# lgb_params4['min_data_in_leaf'] = 500         # min_data_in_leaf
...
lgbsub.to_csv(base_path + 'lgb_esm_submission_zusmani.csv', index=False)


df1 = pd.read_csv(base_path + 'lgb_esm_submission_zusmani.csv')
df2 = pd.read_csv(base_path + 'xgb_submission_zusmani.csv')
df2.columns = [x+'_' if x not in ['id'] else x for x in df2.columns]
blend = pd.merge(df1, df2, how='left', on='id')
for c in df1.columns:
    if c != 'id':
        blend[c] = (blend[c]*5)  + (blend[c+'_']*5)
blend = blend[df1.columns]
blend['target'] = (np.exp(blend['target'].values) - 1.0).clip(0,1)
blend.to_csv(base_path + 'output_zusmani.csv', index=False, float_format='%.6f')
