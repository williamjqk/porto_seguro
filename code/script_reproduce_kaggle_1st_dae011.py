...
model_name = 'porto_seguro_dae010'
...
train_index, test_index = next(skf.split(y_0, y_0))
i1 = 0
for i1, (train_index, test_index) in enumerate(skf.split(y_0, y_0)):
    ...
    n_steps *= n_epochs#1
    # tn_l.append(time.time());print(f"Time cost of {len(tn_l)}: {tn_l[-1] - tn_l[-2]}")
    ...
    # def batch_generator(X_0, y_0, batch_idx_gen):
    #     while True:
    #         idx_batch = next(batch_idx_gen)
    #         yield (X_0[train_index][idx_batch], y_0[train_index][idx_batch])
    # batch_gen = batch_generator(X_0, y_0, batch_idx_gen)
    #
    # # tn_l.append(time.time());print(f"Time cost of {len(tn_l)}: {tn_l[-1] - tn_l[-2]}")
    #
    # batch_gen_func = lambda : batch_gen
    # # def batch_gen_func():
    # #     batch_idx_inner = batch_idx_generator(y_train_0, batch_size=batch_size)
    # #     batch_gen = batch_generator(X_0, y_0, batch_idx_gen)
    # #     return batch_gen

    file_tr = os.path.join(data_path, 'mjahrer_1st_train.dataset')

    def batch_gen_func(file_tr=file_tr, train_index=train_index):
        batch_idx_gen = batch_idx_generator(y_train_0, batch_size=batch_size)
        def batch_generator_h5(file_tr, train_index, batch_idx_gen):
            dataset_tr_inner = HDFDataSet(file_tr, chunk_size=batch_size)
            while True:
                idx_batch = next(batch_idx_gen)
                mask = np.zeros(X_0_shape[0], dtype=int)
                mask[train_index[idx_batch]] = 1
                X_batch = dataset_tr_inner['dae_hidden_feature'][train_index[idx_batch]]
                y_batch = dataset_tr_inner['label'][train_index[idx_batch]]
                yield (X_batch, y_batch)
        batch_gen = batch_generator_h5(file_tr, train_index, batch_idx_gen)
        return batch_gen

    from PPMoney.core.data import DataQueue
    batch_queue = DataQueue(batch_gen_func, capacity=1024, num_worker=10)
    batch_queue.start()

    # next(batch_queue.buffer())
    ...
    for step in range(n_steps):
        ...
        X_batch, y_batch = next(batch_queue.buffer())

        tn_l.append(time.time());print(f"step#{step} read batch, Time cost of {len(tn_l)}: {tn_l[-1] - tn_l[-2]}")
        result, loss_step, _ = sess.run([merge_op, loss, train_op],
                                {tf_x: X_batch,
                                 ...
                                 bn_phase: True})
        tn_l.append(time.time());print(f"step#{step} sess.run, Time cost of {len(tn_l)}: {tn_l[-1] - tn_l[-2]}")


        if step % 100 == 0:
            writer.add_summary(result, step)

            y_train_pred = predict_by_minibatch(X_0[train_index], 20000)
            # y_train_pred = sess.run(y_pred_v, {tf_x: X_train_0})[:,1] # FIXME: X_train_0 is too large
            auc_train = roc_auc_score(y_0[train_index], y_train_pred)
            # ks_train = ks_score(y_train_0, y_train_pred)
            gini_train = gini_mlp(y_train_pred, y_0[train_index])
            # y_eval_pred = sess.run(y_pred_v, {tf_x: X_test})[:,1]
            y_eval_pred = predict_by_minibatch(X_0[test_index], 20000)
            auc_eval = roc_auc_score(y_0[test_index], y_eval_pred)
            # ks_eval = ks_score(y_test, y_eval_pred)
            gini_eval = gini_mlp(y_eval_pred, y_0[test_index])

            gini_eval_l.append(gini_eval)
            if np.argmax(gini_eval_l) == len(gini_eval_l)-1:
                saver.save(sess, os.path.join(model_path,'model.ckpt'))

            tn_l.append(time.time());print(f"step#{step} predict, Time cost of {len(tn_l)}: {tn_l[-1] - tn_l[-2]}")
