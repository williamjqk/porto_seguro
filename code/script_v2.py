...
for i, (train_index, test_index) in enumerate(skf.split(X, y)):
    ...
    train_pred = xgb_model.predict(xgb.DMatrix(train[features].iloc[test_index].values),
...
for i, (train_index, test_index) in enumerate(skf.split(X, y)):
    ...
    train_pred = lgb_model.predict(train[features].iloc[test_index].values,
                        num_iteration=lgb_model.best_iteration)
    sub_train['target'].iloc[test_index] = train_pred
# cv record: 0.2773, 0.2837, 0.2612, 0.2870, 0.2758
...
cv_test = transform_df(test)
cv_train = transform_df(train)

sub_test = test['id'].to_frame()
sub_test['target']=0
sub_train = train['id'].to_frame()
sub_train['target']=0

for i, (train_index, test_index) in enumerate(skf.split(X, y)):
    print(' cbt kfold: {}  of  {} : '.format(i+1, kfold))
    x1, x2 = cv_train.iloc[train_index], cv_train.iloc[test_index]
    y1, y2 = cv_train['target'].iloc[train_index], cv_train['target'].iloc[test_index]
    # x1, x2 = train.iloc[train_index], train.iloc[test_index]
    # y1, y2 = train['target'].iloc[train_index], train['target'].iloc[test_index]
    ...
    sub_test_temp = model3.predict_proba(cv_test[col])[:,1]
    sub_test_temp = (np.exp(sub_test_temp) - 1.0).clip(0,1)
    sub_test['target'] += sub_test_temp / (kfold)

    sub_train_temp = model3.predict_proba(cv_train[col].iloc[test_index])[:,1]
    sub_train_temp = (np.exp(sub_train_temp) - 1.0).clip(0,1)
    sub_train['target'].iloc[test_index] = sub_train_temp
...
sub_test[['id','target']].to_csv(base_path + 'test_catboost_submission.csv', index=False, float_format='%.5f')
sub_train[['id','target']].to_csv(base_path + 'train_catboost_submission.csv', index=False, float_format='%.5f')
...
plt.savefig(base_path+'catboost_feature_importance.png')
# catboost cv record:
hehehe = 'OK'
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(target, target)):
    ...
    sub_preds_train[val_idx] = clf.predict(xgb.DMatrix(trn_df.iloc[val_idx]), ntree_limit=clf.best_ntree_limit)
